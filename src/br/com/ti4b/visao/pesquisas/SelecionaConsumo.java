 ...
 * @author StreamRead
 */
public final class SelecionaConsumo extends javax.swing.JInternalFrame {
    RelatoriosDAO funcao_Relatorios = new RelatoriosDAO();
    ConsumoDAO funcoes_Consumo = new ConsumoDAO();
    List<Consumo> ListConsumos = new ArrayList<>();
    FiltrosDeTabelas filtros = new FiltrosDeTabelas();
    MenuPrincipal m;
    CadastroConsumo consumo;

    public SelecionaConsumo() {
        initComponents();
    }

    public SelecionaConsumo(CadastroConsumo consumo, MenuPrincipal m) {
        initComponents();
        this.m = m;
        this.consumo = consumo;
        povoaTabela();
    }
    public SelecionaConsumo(MenuPrincipal m) {
        initComponents();
        this.m = m;
        povoaTabela();
    }
    ...
    public void povoaTabela() {
       ...
       tabelaPesquisa.getColumnModel().getColumn(4).setPreferredWidth(100);
        ...
        if (comboTipo.getSelectedIndex() == 0) {
            ListConsumos = funcoes_Consumo.pesquisa_consumo(dtm, 1);
        }
        if (comboTipo.getSelectedIndex() == 1) {
            ListConsumos = funcoes_Consumo.pesquisa_consumo(dtm, 2);
        }
    ...
    private void initComponents() {
        ...
        tabelaPesquisa.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Descrição", "Quantidade", "Data de Consumo", "Categoria"
            }
        ) {
        ...
        jLabel1.setText("SELECIONE O ALIMENTO CONSUMIDO");
    ...
    private void SelecionarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SelecionarActionPerformed
         if (tabelaPesquisa.isRowSelected(tabelaPesquisa.getSelectedRow())) {
            for (int i = 0; i < ListConsumos.size(); i++) {
                if (String.valueOf(ListConsumos.get(i).getCod_Consumo()).equals(String.valueOf(tabelaPesquisa.getValueAt(tabelaPesquisa.getSelectedRow(), 0)))) {
                    Consumo ent_Consumo = ListConsumos.get(i);

                    if (consumo != null) {
                        if (comboTipo.getSelectedIndex() == 0) {
                            consumo.setConsumo(ent_Consumo, 1);
                        }

                        if (comboTipo.getSelectedIndex() == 1) {
                            consumo.setConsumo(ent_Consumo, 2);
                        }
                    } else {
                        CadastroConsumo cadastroConsumo = new CadastroConsumo(m);
                        MenuPrincipal.jDesktopPane1.add(cadastroConsumo);
                        cadastroConsumo.setVisible(true);
                        cadastroConsumo.setPosicao();
                        if (comboTipo.getSelectedIndex() == 0) {
                            cadastroConsumo.setConsumo(ent_Consumo, 1);
                        }

                        if (comboTipo.getSelectedIndex() == 1) {
                            cadastroConsumo.setConsumo(ent_Consumo, 2);
                        }
                    }
                }
            }
   ...
   if (tabelaPesquisa.isRowSelected(tabelaPesquisa.getSelectedRow())) {
        ...
        if (comboTipo.getSelectedIndex() == 0) {
            funcao_Relatorios.relatoriosReproducao("relatorioConsumoIndividualconcentrado", cod);
        } else if (comboTipo.getSelectedIndex() == 1) {
            funcao_Relatorios.relatoriosReproducao("relatorioConsumoIndividualVolumoso", cod);
        }
   }
        // TODO add your handling code here:
